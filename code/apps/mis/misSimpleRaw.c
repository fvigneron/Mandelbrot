...
static void initCircles(mpfr_t radius) {
    mpfr_t th;
    mpfr_init2(th, 128);
    for (int i = 0; i < MIS_RAW_ANG; i++) {
        ...
        mpc_mulr(sc + i, sc + i, radius);
    }
    mpfr_clear(th);
    
    ldbl r = mpfr_get_ld(radius, MPFR_RNDN);
    // fp80 version of circle targets
    for (int i = 0; i < MIS_RAW_ANG; i++) {
        scl[i].x = r * cosl(2 * PI * i / MIS_RAW_ANG);
        scl[i].y = r * sinl(2 * PI * i / MIS_RAW_ANG);
    }
...
static inline bool mis_raw_find_misc(mpc res, mpc sp, int pp, int per, int ang, int iter) {
    ...
    bool ok = mandel_miss_root_ref(res, sp, &sc[(ang + MIS_RAW_ANG) % MIS_RAW_ANG], pp, per, iter, LC_REF);
...
static bool checkAndAddRoot(int pp, int per, mpc cr) {
    ...
    // check all the other cases (not roots of p_{pp-1,per})
    for (int dper = per; dper >= 1; dper--) {
        if(per % dper != 0) {
            continue;
        }
        
        if(mandel_is_mis(c, pp, dper, MIS_RAW_PROOF)) {
            nset_add(divSet, u);
...
static int accelRoot(fp80 v, fp80 sp, int pp, int per, int iter, double err) {
    ...
    for (int i = 0; i < iter; i++) {
        ntOk = mandel_miss_ntl(nt, op, z, pp, per) >= MIN_DER2;
...
static inline bool refine(mpc rc, mpc sp, int pp, int per, int qIter, int qPrec, int rIter, int rPrec) {
    ...
    for (int i = 0; ! conv && i < qIter; i++) {
        ...
        mandel_miss_nt_sol(nt, c, d, pp, per);
        mpc_sub(c, c, nt);
    ...
    for (int i = 0; ! conv && i < rIter; i++) {
        mandel_miss_nt_sol(nt170, c170, d170, pp, per);
...
static bool next_lc_simple(int pp, int period, mpc ep, fp80 ep8, mpc sp, fp80 sp8, fp80 prevSp8, int ang) {
    int n = pp + period;
    int iter = 3 * n / 4;
...
//    }
    ...
    // perform at least a few steps with fp80, then handover to mp
    ...
    do {
        *op8 = *p8;
        d2 = mandel_miss_ntl(nt, p8, t, pp, period);
        fp80_sub(p8, p8, nt);
        m2 = fp80_mod2(nt);
...
///
/// @return @ref true if successfull, @ref false otherwise
static bool roots_simple(int pp, int per, mpc sp, mpc ep, int spCount, int step) {
    int n = pp + per;
    ...
    *op8 = *p8;
    ...
    for (i = spCount - 1; i >= 0 && ok; i--) {
        ...
        for (int j = step - 1; j >= 0 && ok; j--) {
            *tp8 = *p8;
            ok = ok && next_lc_simple(pp, per, lcp, p8, lcp, p8, op8, step * i + j);
            *op8 = *tp8;
        }
    }
    
    // refine lcp, it may come from fp80 computation
    ok = ok && mandel_miss_root_ref(lcp, lcp, sc, pp, per, LC_ITER, LC_REF);
...
static bool find_simple_mis_raw(int pp, int per, int start, int end) {
    char hostname[_SC_HOST_NAME_MAX + 1];
    gethostname(hostname, _SC_HOST_NAME_MAX + 1);
    
    int cpu = get_cpu_id();
    
    printf("\nComputing simple pre-periodic points of type (%d, %d), jobs %d to %d (max jump %.4Lf) on %s with cpu %d:\n\n",
    ...
    for (int job = start; ok && job < end; job++) {
        ...
        int spCount = n + 1 < MISS_MIN_PER ? 1 << n :
        ...
        for (int seg = 0; ok && seg < jobSize; seg++) {
            ...
            ok = roots_simple(pp, per, sp, ep, spCount, MIS_RAW_STEP);
...
int mis_simple_raw_main(int argc, const char * argv[]) {
    ...
    return ! find_simple_mis_raw(pp, per, start, end);
}
